...
- [x] A — Dock (Days 2-3) · MEs: ME-02/03/04/05/06 PASS
      dock/widget.py + dock/model.py done; boots as DOCK (geo 320x72, strut honored), 4 default pins from dock.json, running indicators wired to ClientListWatcher (no crash on open/close), launch/focus, pin/unpin/reorder via right-click + persistence, grid button -> menu_requested. Idle CPU 0.37% avg (8s pidstat), RSS 146 MB. Visual polish done: drag-and-drop reorder, hover-scale animation, improved indicator dot + halo, dock drop-shadow.
...
- [x] C — Widget engine + CMS (Days 6-7) · MEs: ME-07 PASS, ME-08 PASS
      widgets/engine.py (WidgetPlugin ABC + auto-discovery + execute_action + layout.json), 4 plugins (greeting_clock, news, quick_tiles, carousel), widgets/desktop.py (DESKTOP-layer grid + WidgetPanel slide-in overlay), widgets/cms.py offline-first wired in. Boots clean: idle CPU 0.00%, RSS 141 MB; offline (dead endpoint) serves cache, no crash. WidgetPanel: 320px slide-in from right edge, 180ms animation, each plugin shown as card with icon+name+desc+Add/Remove button (live, no modal). PENDING: human visual confirm.
...
- [x] Gate G1 — Fresh-VM .deb proof + Flatpak match (Day 3) · **D2 CLOSED, D6 CLOSED**
      Ran on a freshly RESTORED snapshot (scripts/gate-g1.sh, with a cleanliness SENTINEL that aborts unless the precondition holds, so no false pass). Finding: 00-clean was snapshotted after Day-0, so it carries the shared deps; on Ubuntu 24.04 + LxQt python3-pyqt5/xdg/requests + papirus ship with the base session (meteo-qt/speechd/cloud-init/LxQt), and **python3-xlib is the only dep unique to us**. Created the clean precondition (remove xlib), then `apt install ./deb` **pulled python3-xlib automatically** and installed cleanly; all 5 deps resolved, launcher+autostart+themes placed, **login-to-visible 0.621 s**, RSS 127 MB, no errors, DESKTOP+DOCK present. D6: installed org.gnome.Calculator (Flatpak), live window WM_CLASS `gnome-calculator` now resolves to `org.gnome.Calculator`. Two real bugs caught + fixed by the gate (see ME log).
...
| G1 | PASS | 2026-06-12 | Fresh-VM proof on restored snapshot. `apt install ./deb` auto-pulled python3-xlib (the only app-unique dep on LxQt) + installed cleanly; 5/5 deps resolved; login-to-visible **0.621 s**; no errors. Flatpak D6: live `gnome-calculator` WM_CLASS -> org.gnome.Calculator. **Bug 1 (packaging, HIGH):** postrm did unconditional `rm -rf /usr/lib/jiopc-home`; dpkg runs old postrm `upgrade` AFTER new files unpack, so reinstalls/upgrades self-destructed (fresh installs masked it). Fixed: guard `case "$1" in remove|purge)`. **Bug 2 (matching):** Flatpak Calculator has no StartupWMClass and reports WM_CLASS=`gnome-calculator` (its `--command=`), which our app-id/name keys missed. Fixed: `_exec_basename()` adds the exec/`--command=` basename as a match key. Both re-verified on VM |
...
- RESOLVED (Gate G1, 2026-06-12): fresh-VM .deb proof done on a restored snapshot. Note the "00-clean" snapshot is NOT dependency-free (taken post-Day-0); on Ubuntu 24.04 + LxQt the only app-unique dep is python3-xlib, which apt pulls automatically. gate-g1.sh now has a sentinel so a non-clean precondition can never record a false pass. Future: bake SSH key + passwordless sudo into a fresh baseline snapshot so the gate runs without manual re-setup (the restore wipes port-forward/key/sudo each time).
...
- Visual polish II (2026-06-12): Rich background with radial glow accents (top-left blue, bottom-right navy) + dot-grid texture, all painted in DesktopLayer.paintEvent. WidgetPanel: 320px slide-in overlay, 180ms OutCubic animation, plugin cards with icon+name+desc+Add/Remove (live, no modal dialog, persists immediately). App menu redesigned: single QScrollArea containing Recent/Recommended/All Apps in one continuous scroll; fixed header (search + chips) stays pinned. All Apps grid replaced QListView with _TileGrid (QGridLayout). Verified: FIRST_PAINT 150ms, RSS 141MB, idle CPU 0.00% (8s). PENDING: human visual confirm on VM.
